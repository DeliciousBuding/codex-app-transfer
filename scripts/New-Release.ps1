...
function Get-OrCreateSigningKey {
    ...
    Copy-Item -LiteralPath $publicPath -Destination (Join-Path $ReleaseDir "Codex-App-Transfer-release-public.pem") -Force
    return $privatePath
}
...
function Add-MacPlatformAssets {
    ...
    foreach ($extension in @("pkg", "dmg")) {
        $assetPath = Join-Path $MacDir "Codex-App-Transfer-v$Version-macOS-$Arch.$extension"
...
$folderDist = Join-Path $distDir "Codex-App-Transfer"
$oneFileExe = Join-Path $distDir "Codex-App-Transfer.exe"
$setupExe = Join-Path $root "Codex-App-Transfer-Setup-$Version.exe"

Set-Location $root
New-Item -ItemType Directory -Force -Path $releaseDir | Out-Null
foreach ($pattern in @(
    "Codex-App-Transfer-v*-Windows-*",
    "Codex-App-Transfer-v*-macOS-*",
...
$folderExe = Join-Path $folderDist "Codex-App-Transfer.exe"
...
$portableZip = Join-Path $releaseDir "Codex-App-Transfer-v$Version-Windows-Portable.zip"
...
$releaseExe = Join-Path $releaseDir "Codex-App-Transfer-v$Version-Windows-x64.exe"
Copy-Item -LiteralPath $oneFileExe -Destination $releaseExe -Force

$releaseSetup = $null
if (Test-Path -LiteralPath $setupExe) {
    $releaseSetup = Join-Path $releaseDir "Codex-App-Transfer-v$Version-Windows-Setup.exe"
...
$latest = [ordered]@{
    ...
    notes = "Windows release for Codex App Transfer v$Version."
    update_protocol = 1
    minimum_supported_version = "1.0.0"
    platforms = $platforms
    signature = [ordered]@{
        algorithm = "RSA-CSP-BLOB-SHA256"
        public_key = "Codex-App-Transfer-release-public.pem"
