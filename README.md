...
**Grok Web 反代适配器(实验性,builtin preset)**
- 新增 `apiFormat=grok_web` + `authScheme=grok_cookie`,反代 grok.com Web 后端(`POST /rest/app-chat/conversations/new`)。用户用 **SuperGrok / X Premium+** 订阅 cookie 鉴权,通过 grok-4.20 computer-use special agent 模型走多轮 chat
- **UI 极简(Plan A)**:builtin "Grok (Web 反代)" preset 卡片,**仅必填 `sso` 一个 JWT 字段**;`x-statsig-id` 每次请求**后端动态生成**伪 Statsig blob(参考 [`chenyme/grok2api`](https://github.com/chenyme/grok2api) `_statsig_id` 算法,grok.com 服务端只校验 base64 格式不校验内容);`sso-rw` 自动复用 `sso`(实测同 JWT);User-Agent / Origin / Referer / Sec-Ch-* 后端硬编码默认值。`cf_clearance` / 显式 statsig override / UA override 走"高级选项"折叠区,**默认无需填写**。敏感字段(cookies + statsigId)按 apiKey 同等级 mask,public_provider 返 `hasGrokWeb: bool` 标记
- **Connector 走 server-side state + 黑名单机制**(实测 SuperGrok 三次 cURL 抓包 verified):client 请求**只传 `disabledConnectorIds: []` 黑名单**,**不传** `connectorIds` / `connectors` / `toolOverrides`。用户在 `grok.com/connectors` UI 注册并启用的 connector(Google Drive / Gmail / Notion / GitHub / 自定义 MCP server 等)在 chat 时由 grok.com 后端自动注入模型上下文。MCP 调用通过 `call_connected_tool` wrapper,namespace 用 `<connector_displayName>___<tool_name>` 三下划线分隔
- 新独立子模块 `crates/adapters/src/grok_web/`(thin Adapter trait + types/auth/request/response/parent_response)+ `mapper/grok_web.rs`(RequestMapper / ResponseMapper trait 接线,符合 Phase 4 RFC 规范)。响应侧 SSE 用 `stream::unfold` + `VecDeque<Bytes>` pending events 缓冲(跟 `gemini_native` 同 pattern,无新依赖),处理 newline-delimited JSON 帧 → OpenAI Responses event 双向转换
- ⚠️ **实验性 + TOS 灰色**:grok.com 使用条款不允许把 Web 端能力商业化转 API,本探索严格限定**个人使用 + 本机 SuperGrok 账号**,**不应**作为对外服务发布。grok.com 前端协议字段持续变化(2025 年下半年至 2026 年间已多次调整 SSE 帧 / connector schema),cookie 频繁机器使用可能触发账号风控,用户自担风险
- **当前能力范围**:✅ 单轮 / 多轮文本 chat(`final` token → `response.output_text.delta`)✅ 鉴权头组装 + 动态 statsig 生成 ✅ 流式增量 ✅ Connector 黑名单语义 ✅ `parentResponseId` 跨轮 DAG tracker(LRU bounded 1000 entries)✅ reasoning summary part / text / done 全 lifecycle ✅ Web Search / Connector / Collection / RAG Search citations(末尾追加 annotation)✅ codeExecutionResult → reasoning markdown fenced code block。⏳ 后续:inline `[N]` 精确位置 citation(`<grok:render>` parser)、`cardAttachment` 帧、真账号 E2E 验证
- **协议反向工程 + 算法借鉴致谢**:本适配器协议事实(endpoint / payload 24 字段 / SSE 帧 schema / `messageTag` 全集 / Connector 黑名单机制 / `call_connected_tool` MCP wrapper / `___` namespace 命名规则)整合自三处来源:(1) [`chenyme/grok2api`](https://github.com/chenyme/grok2api) Python 项目(4.8k⭐)近一年累积的反向工程产出 — endpoint table、payload 字段集骨架、SSE 帧解析模式 `StreamAdapter`、citations annotation 算法 PR #468、PR #226 `toolOverrides` passthrough 失败教训、**`dynamic_statsig` 动态生成 `x-statsig-id` 算法**(`app/dataplane/proxy/adapters/headers.py::_statsig_id`,本仓库 Rust 端口)、`sso={t}; sso-rw={t}` 单 JWT 双写习惯 — **代码独立实现,不依赖其二进制**;(2) grok.com 前端 76 个 webpack chunks 反编译;(3) 本仓库 SuperGrok 账号 2026-05-11 三次请求 cURL + 五次响应 SSE 抓帧实测
- 40+ 单测覆盖 wire types(24 字段稳定 + ToolCall enum + messageTag 全集)+ auth(cookie 组合 + 仅 sso 必填 + sso-rw 自动复用 + 动态 statsig base64 解码验证 + uuid v4 / traceparent 生成)+ request(messages → message + parentResponseId 锚定 + reasoning effort 映射)+ response(SSE chunk 切线防御 + thinking 过滤 + 流断 completed 补)+ parent_response tracker(LRU)
...
**Grok Web reverse-proxy adapter (experimental scaffold, not yet released)**
- New `apiFormat=grok_web` + `authScheme=grok_cookie` — reverse-proxies the grok.com Web backend (`POST /rest/app-chat/conversations/new`). Users authenticate with their **SuperGrok / X Premium+** subscription cookies (`sso` + `sso-rw` + `cf_clearance`, etc.) plus a Statsig ID, chatting through the grok-4.20 computer-use special-agent model with multi-turn support
- **Connector uses server-side state + denylist semantics** (verified by three SuperGrok cURL captures): client requests **only carry `disabledConnectorIds: []` (a denylist)** and **never include** `connectorIds` / `connectors` / `toolOverrides`. Connectors the user registered and toggled on in `grok.com/connectors` UI (Google Drive / Gmail / Notion / GitHub / custom MCP servers, etc.) are injected by the grok.com backend into the model context automatically. MCP calls go through a `call_connected_tool` wrapper with namespaces of the form `<connector_displayName>___<tool_name>` (three underscores)
- New `crates/adapters/src/grok_web/` submodule (thin Adapter trait + types/auth/request/response/parent_response) plus `mapper/grok_web.rs` (RequestMapper / ResponseMapper trait wiring per the Phase 4 RFC). The response side uses `stream::unfold` + `VecDeque<Bytes>` pending-event buffering (same pattern as `gemini_native`, no new deps), translating newline-delimited JSON frames → OpenAI Responses events bidirectionally
- ⚠️ **Experimental + TOS gray area**: grok.com's terms of service do not permit commercializing Web-end capabilities as an API. This exploration is strictly limited to **personal use on a local SuperGrok account** and **must not** be deployed as a public service. The grok.com frontend protocol fields evolve continuously (SSE frame / connector schema changed multiple times between H2 2025 and 2026), and frequent automated cookie use may trigger account risk controls — users assume all risk
- **Current R3 PoC scope**: ✅ single-turn text chat (`final` token → `response.output_text.delta`), ✅ auth header assembly, ✅ streaming increments, ✅ connector denylist semantics, ✅ `parentResponseId` DAG tracker scaffolded. ⏳ Stacked PRs will extend: `tool_usage_card` / `raw_function_result` frames → function_call + url_citation annotations, thinking tokens → `reasoning.delta`, parentResponseId recording closure, desktop_app UI form
- **Protocol reverse-engineering acknowledgements**: this adapter's protocol facts (endpoint / 24-field payload / SSE frame schema / full `messageTag` set / connector denylist mechanism / `call_connected_tool` MCP wrapper / `___` namespace convention) integrate three sources: (1) [`chenyme/grok2api`](https://github.com/chenyme/grok2api)'s ~year of accumulated reverse-engineering (endpoint table, payload field-set skeleton, `StreamAdapter` SSE parsing pattern, citations annotation algorithm from PR #468, PR #226's `toolOverrides` passthrough failure lessons) — **code is independently re-implemented and does not depend on the upstream binary**; (2) decompilation of grok.com's 76 frontend webpack chunks; (3) this repository's own 2026-05-11 SuperGrok captures (three request cURLs + five response SSE traces)
- 30+ unit tests cover wire types (24-field stable set + ToolCall enum + full messageTag set), auth (cookie assembly + statsig injection + UUID v4 / traceparent generation), request (messages → message field + parentResponseId anchoring + reasoning effort mapping), response (SSE chunk mid-line defence + thinking filtering + soft-stop completed defence), and the parent_response tracker
