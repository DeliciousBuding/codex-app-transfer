...
**Grok Web 反代适配器(实验性 scaffold,未进 release)**
- 新增 `apiFormat=grok_web` + `authScheme=grok_cookie`,反代 grok.com Web 后端(`POST /rest/app-chat/conversations/new`)。用户用 **SuperGrok / X Premium+** 订阅 cookie(`sso` + `sso-rw` + `cf_clearance` 等)+ Statsig ID 鉴权,通过 grok-4.20 computer-use special agent 模型走多轮 chat
...
- **当前 R3 PoC 范围**:✅ 单轮文本 chat(`final` token → `response.output_text.delta`)✅ 鉴权头组装 ✅ 流式增量 ✅ Connector 黑名单语义 ✅ `parentResponseId` DAG tracker 就绪。⏳ 后续 stacked PR 扩展:`tool_usage_card` / `raw_function_result` 帧 → function_call + url_citation annotation、thinking token → `reasoning.delta`、parentResponseId 录入闭环、desktop_app UI 表单
- **协议反向工程致谢**:本适配器协议事实(endpoint / payload 24 字段 / SSE 帧 schema / `messageTag` 全集 / Connector 黑名单机制 / `call_connected_tool` MCP wrapper / `___` namespace 命名规则)整合自三处来源:(1) [`chenyme/grok2api`](https://github.com/chenyme/grok2api) Python 项目近一年累积的反向工程产出(endpoint table、payload 字段集骨架、SSE 帧解析模式 `StreamAdapter`、citations annotation 算法 PR #468、PR #226 `toolOverrides` passthrough 失败教训)— **代码独立实现,不依赖其二进制**;(2) grok.com 前端 76 个 webpack chunks 反编译;(3) 本仓库 SuperGrok 账号 2026-05-11 三次请求 cURL + 五次响应 SSE 抓帧实测
- 30+ 单测覆盖 wire types(24 字段稳定 + ToolCall enum + messageTag 全集)+ auth(cookie 组合 + statsig 注入 + uuid v4 / traceparent 生成)+ request(messages → message + parentResponseId 锚定 + reasoning effort 映射)+ response(SSE chunk 切线防御 + thinking 过滤 + 流断 completed 补)+ parent_response tracker
